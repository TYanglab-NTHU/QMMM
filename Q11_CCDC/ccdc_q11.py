...
if VerboseFlag == 2:
  printf("Step %i - Distant neighbour search\n", stp_cnt)
...
stp_cnt += 1
shell.append([]) # create shell[3] for neighboring rings

if VerboseFlag==2:
  printf("Step %02i - rings containing neighbours atoms\n", stp_cnt)
  printf("    atom  smallest ring\n")
for ne in my_clean_list(dnts['close']+dnts['dist']):
  if VerboseFlag==2:
...
itl = my_clean_list(dnts['close'] + dnts['dist'] + dnts['nrings'])
...
qm += shell[stp_cnt-1]
qm = my_clean_list(qm)
dnts['cArings'] = shell[stp_cnt-1]
dnts.update()
summarize_step(stp_cnt, dnts['cArings'], qm, "central aromatic rings")
...
while bool(True):   # infinite loop for the iterative search
  ...
  if len(new_at) == 0:
    ...
    shell[stp_cnt-1] += new_at
    shell[stp_cnt-1] = my_clean_list(shell[stp_cnt-1])
    itl = my_clean_list(itl + shell[stp_cnt-1])
    new_at = []
    qm += shell[stp_cnt-1]
    qm = my_clean_list(qm)
if VerboseFlag == 2:
  printf("  %i iterative search rounds needed\n", it_cnt-1)
dnts['gArings'] = shell[stp_cnt-1]
dnts.update()
