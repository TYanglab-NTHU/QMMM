...
# downloads.ccdc.cam.ac.uk/documentation/API/descriptive_docs/molecule.html    #
################################################################################
...
def con_unit(mymol, at1, at2, at3):
  ...
  # if at2.is_aromatic: return(0)  # not a candidate
  ...
  for a2 in mol.atoms: # loop over all atoms
    ...
    if dist < 3.0 and a2 not in qm:
      ...
      if dist<=vdw_sum:
        ...
        if InMolBonds(mol, a1, a2)==-1 and bool(False):
...
for at1 in shell[1]: # Loop over direct neighbors
  if len(at1.rings) > 0:
    continue
  # Build triple for the search
  for at2 in at1.neighbours:
    ...
    for at3 in at2.neighbours:
      ...
      if VerboseFlag==2:
        print(MyLabel(at1), MyLabel(at2), MyLabel(at3), con_level)
exit()
