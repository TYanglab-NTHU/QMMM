...
def output_generation(xyz):
  ...
  ################################################################################
  # 02b - additional search based on distance                                    #
  ################################################################################

  qm_cnt += 1
  new_at = []
  bnd_cnt = 0
  if VerboseFlag > 0: printf("02b - Neighbours by distance (<= 2.75 A)\n")
  ...
  qm = my_clean_list(qm)
  Zneighbors += new_at
  Zneighbors = my_clean_list(new_at)
  if VerboseFlag > 0:
    printf("  %i new atoms found\n", len(new_at))
    if len(new_at) > 0:
      my_atom_list(new_at, "  ")
    printf("  %i new single bonds added\n", bnd_cnt)
    fname = "qm_at%02i.xyz" % qm_cnt
    printf("  Write %s\n", fname)
    list_xyz(qm, fname, "Center, direct & distance neighbours")
  ...
  while len(new_at) != 0:
    ...
    # clean up the list of aromatic rings passed from the last step
    if VerboseFlag > 0:
    ...
    new_at = my_clean_list(new_at)
    if VerboseFlag > 0:
      printf("  %i new MM atoms detected\n", len(new_at))
      if len(new_at) == 0:
        qm_cnt -= 1
        break
      if len(new_at) > 0:
        my_atom_list(new_at, "  ")
    # add newly found atoms to the existing lists
    shell[2] += new_at
    shell[2] = my_clean_list(shell[1])
    qm += new_at
    qm = my_clean_list(qm)
    # write file to document the growth of the QM core
    if VerboseFlag > 0:
      fname = "qm_at%02i.xyz" % qm_cnt
      printf("  Write %s\n", fname)
      list_xyz(qm, fname, "Center, direct neighbour rings")
  ...
  ################################################################################
  # 02c - looking for 1 atom links between QM atoms
  ################################################################################

  if VerboseFlag > 0:
    printf("04a - 1 Atom MM Bridges between QM Atoms\n")
  qm_cnt += 1
  new_at = [] # list for new atoms
  if VerboseFlag == 2:
    printf("  imedeate search results\n")
  for sa in qm:
    for ea in qm:
  ...
  if VerboseFlag > 0:
    fname = "qm_at%02i.xyz" % qm_cnt
    printf("  Write %s\n", fname)
    list_xyz(qm, fname, "1 atom bridges added")
  ...
  if VerboseFlag > 0:
    printf("04b - 1 Atom MM Bridges between QM Atoms\n")
  qm_cnt += 1 # QM search step counter
  new_at = [] # list for new atoms

  # enter the loops to build possible bridges
  for b0 in qm:
  ...
  ################################################################################
  # XXx - adding single hydrogen atoms
  ################################################################################
