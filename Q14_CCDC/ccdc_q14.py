...
# 16.09.2024
# - Clean up completed in the days before
# - spiro atom fixed for direct neighbours rings
# 17.09.2024
# - exo double bonds added
# - move to ccdc_q13.py
    ...
    for qa in qm: # loop over quantum core
      for na in qa.neighbours:
        if na in qm:
          continue
        if VerboseFlag==2:
          printf("  %-6s", MyLabel(qa))
          printf("  %-6s", MyLabel(na))
        bidx = InMolBonds(mol, qa.index, na.index)
        btype = mol.bonds[bidx].bond_type
        if VerboseFlag==2:
          printf("  %-6s", mol.bonds[bidx].bond_type)
        if mol.bonds[bidx].bond_type != 1:
          new_atoms.append(na)
        if VerboseFlag==2:
          if mol.bonds[bidx].bond_type != 1:
            printf("  add %s", MyLabel(na))
          printf("\n")
    if len(new_atoms) == 0:
      break
